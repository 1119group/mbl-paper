...
The eigenstate thermalization hypothesis (ETH) describes how an isolated
self-interacting quantum system can thermalize under its own internal dynamics
in agreement with conventional equilibrium statistical mechanics. However, a
subset of these systems fail to thermalize on any time scale due to disorder
and are categorized under the phenomenon known as many-body localization (MBL).
Experimental evidence(cite experimental stuff) of the MBL phase and a proof
under the assumption of limited level attraction\cite{imbrie2014} has helped to
drive recent interest on the subject.


The existence of the ETH and MBL phases suggests a phase transition; however,
pinpointing this transition is difficult since the thermal phase near the
transition can appear to not thermalize on accessible time scales. Random
disorder introduces rare Griffiths regions\cite{vosk_theory2014, potter2015trans, knap2015}
which may have singular contributions in driving a phase transition, but there
is still limited quantitative understanding about their effects and should be
absent in a quasi-periodic potential. Numerical studies have focused on random
disorder(citation:luitz, etc), but there need for studies on quasi-periodic
disorder(are there any to cite?).
...
Larger size (with up to $N=22$ spins) numerical exact diagonalization (ED)
...
than that obtained by ED studies\cite{luitz2015}. Theoretical\cite{vosk_theory2014}
and numerical studies of low frequency conductivity\cite{agarwal2015, knap2015}
and energy spectra statistics\cite{serbyn2015} have suggested that there is an
intermediate regime with sub-diffusive conductivity and (or) semi-Poisson level
statistics between the ergodic and MBL phases. A consistent picture for
understanding the dynamic phase transition in such a system is still absent.



In this paper, we report eigenstate and time-dependent studies of a spin chain
with a quasi-periodic potential. First, we find a quantum phase transition from
the ergodic phase to the many-body localized phase based on entanglement
entropy and the ratio of the adjacent gaps similar to systems with random
potentials. However, quasi-periodic systems are shown to be more efficient at
localizing interacting states with a smaller critical $h_c \sim 2$. We perform
...
phase demonstrates quasi-periodic oscillations in time for shorter time
(whichplot? Is this expected for finite size?). Both the imbalance and the spin
glass order survives to extremely long time scale as a signature result of the
MBL phase. Our results indicate that quasi-periodic systems share these common
...
\begin{figure}[b]
  ...
  \caption{(Color online) (a) The entropy $S(t)$ as a function of time $t$ for
    weaker quasi-potential $h=0.5-2$ for systems with $L=16$ sites. In this
    log-log plot, we see power-law growth of $S(t)$ w.r.t. $t$ which saturates
    at $(L/2-1)ln(2)$ for sub-critical $h$.
...
\begin{figure}[b]
  ...
  \caption{(Color online) (a) For small $W=1$ and system sizes $L=12-20$, we
    find the $S(t)$ data fits into a nice straight line for $t=1\sim 50$
    indicating power-law growth of $S(t)$. For $t<1$ we see the initial
    expansion of the entropy. When $t$ becomes large,
